...
0. **Intake (only if the user asks for it)** — the intake flow (product-analyst → approved stories → tracker) runs only when the user explicitly requests it (`/dev-team:team-intake`, "turn this into stories", "run the analyst on this"). Otherwise, even when the input is a raw requirements document, go straight to step 1 — tech-lead plans from the document directly. If the document looks story-splittable, you may mention intake exists, in one line, but don't run it.
